...
>
> ```
> Ignition: V8의 인터프리터, 모든 코드를 바이트코드로 변환해 즉시 실행
> TurboFan: V8의 JIT 컴파일러, Ignition이 실행 중 자주 반복되는 핫한 코드 감지 시에만 작동 → 머신코드로 최적화 후 캐싱
>
> 변환 흐름: JS 소스코드 → (파싱) → AST → (Ignition) → 바이트코드 → (TurboFan, 핫코드만) → 머신코드
>
> 바이트코드: 중간 단계 코드, 가상머신이 읽음 / 머신코드: CPU가 직접 읽는 이진코드, 가장 빠름
> ```
...
JavaScript (JIT, V8)
코드 → Ignition(인터프리터) 한 줄씩 해석 → 바이트코드 → 즉시 실행
      ↓ 자주 실행되는 부분 감지
      TurboFan(컴파일러) → 머신코드로 컴파일 → 캐싱
      ↓ 반복 실행: 점점 빨라짐 ✅
...
반복되는 코드가 없으면 JIT가 컴파일할 게 없으니까 그냥 Ignition(인터프리터)만 작동해서 기존 인터프리터처럼 동작. 반복되는 부분을 감지하면 TurboFan이 머신코드로 컴파일 → 반복할수록 빨라짐.
